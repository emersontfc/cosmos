...
module.exports = alpha = async (alpha, m, chatUpdate, store) => {
    try {
        ...
        const wita = moment(Date.now()).tz('Africa/Maputo').locale('id').format('HH:mm:ss z')
        const wit = moment(Date.now()).tz('Africa/Maputo').locale('id').format('HH:mm:ss z')
...
Ele esta  em AFK ${reason ? '\nMotivo : ' + reason : 'Motivo : Nenhum'}
...
m.reply(`
Você parou AFK${user.afkReason ? ' depois de ' + user.afkReason : ''}
...
Lihat list Pesan Dengan ${prefix}listmsg`)
	...
	case 'antiviewonce': case 'antionce':
        ...
        if (!m.key.fromMe && !isCreator) return reply(lang.ownerOnly())
        if (args[0] === "on") {
        	if (global.db.data.chats[m.chat].nsfw) return reply(`Anteriormente ativo`)
        global.db.data.chats[m.chat].nsfw = true
        reply(`NsFw Ativado com sucesso !`)
        } else if (args[0] === "off") {
        	if (!global.db.data.chats[m.chat].nsfw) return reply(`Já esteve desligado antes`)
...
//fim        
	    ...
	    case 'toimage': case 'toimg': {
            ...
            break
            case 'admin': case 'boss': case 'proprietario': {
                alpha.sendContact(m.chat, global.owner, m)
            }
            break
            case 'rules':  case 'regras':{
            ...
            var but = [
                ...
                case 'donasi': case 'donate': case 'apoiar': case 'ajudar':{
            var but = [
          {
            "urlButton": {
...
case 'jadwalshalat':{
	...
	.then(async data =>{
		...
		txt += `• Daerah : ${q}\n\n`
		txt += `• Date : ${moment(new Date()).tz('Africa/Maputo').locale('id').format('HH:mm:ss DD/MM/YYYY')}\n`
